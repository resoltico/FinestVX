...
def transaction_from_mapping(payload: object) -> JournalTransaction:
    ...
    return JournalTransaction(
        reference=_require_text(data["reference"], "transaction.reference"),
        posted_at=_require_datetime(data["posted_at"], "transaction.posted_at"),
        description=_require_text(data["description"], "transaction.description"),
        entries=tuple(
        ...
        state=TransactionState(_require_text(data["state"], "transaction.state")),
        reversal_of=_optional_text(data.get("reversal_of"), "transaction.reversal_of"),
    )
...
def _account_from_mapping(payload: object) -> Account:
    ...
    return Account(
        code=_require_text(data["code"], "account.code"),
        name=_require_text(data["name"], "account.name"),
        normal_side=PostingSide(_require_text(data["normal_side"], "account.normal_side")),
        currency=_require_text(data["currency"], "account.currency"),
        parent_code=_optional_text(data.get("parent_code"), "account.parent_code"),
...
def _book_period_from_mapping(payload: object) -> BookPeriod:
    ...
    return BookPeriod(
        ...
        state=FiscalPeriodState(_require_text(data["state"], "book_period.state")),
    )
...
def book_from_mapping(payload: object) -> Book:
    ...
    return Book(
        code=_require_text(data["code"], "book.code"),
        name=_require_text(data["name"], "book.name"),
        base_currency=_require_text(data["base_currency"], "book.base_currency"),
        ...
        legislative_pack=_require_text(data["legislative_pack"], "book.legislative_pack"),
        accounts=tuple(
